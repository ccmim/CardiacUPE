get_best_phenotype_per_locus <- function(locus) {
  best_locus_row_df <- gwas_across_runs_df %>% filter(region == locus) %>% top_n(n=1, wt=-P)
  exp_id <- best_locus_row_df %>% .$pheno %>% strsplit(., split = "_") %>% .[[1]] %>% .[1]
  run_id <- best_locus_row_df %>% .$run
  z_var <-  best_locus_row_df %>% .$pheno %>% strsplit(., split = "_") %>% .[[1]] %>% .[3]
