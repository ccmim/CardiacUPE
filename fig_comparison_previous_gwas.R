...
colnames(assocs)[1] = "CoMA Ensemble"
...
THRESHOLD <- 9.5
assocs_mtx_capped <- assocs_mtx_capped[(!rownames(assocs_mtx_capped) %in% remove_loci) & assocs_mtx_capped[,"CoMA Ensemble"] > THRESHOLD,]

assocs_mtx_capped <- cbind(
  assocs_mtx_capped, 
  apply(assocs_mtx_capped[,colnames(assocs_mtx_capped) != "CoMA Ensemble"], 1, max)
) 
colnames(assocs_mtx_capped)[29] <- "Best association"
